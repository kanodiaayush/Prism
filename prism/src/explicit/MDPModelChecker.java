...
public class MDPModelChecker extends ProbModelChecker
{
	 ...
	 */
	protected StateValues checkProbPathFormula(Model model, Expression expr, boolean min,
	 ...
	 */
	protected StateValues checkProbPathFormulaSimple(Model model, Expression expr, boolean min,
			Prob0Info prob0Info) throws PrismException
	{
	 ...
	 */
	protected StateValues checkProbUntil(Model model, ExpressionTemporal expr, boolean min,
			Prob0Info prob0Info) throws PrismException
	{
		BitSet b1, b2;
		StateValues probs = null;
	...
	public ModelCheckerResult computeReachProbs(MDP mdp, BitSet remain, BitSet target, boolean min, double init[], BitSet known, Prob0Info prob0info) throws PrismException
	{
		...
		if ((precomp && prob0) || exactSol) {
			no = prob0(mdp, remain, target, min, prob0info);
		} else {
			no = new BitSet();
		}
	...
	protected ModelCheckerResult computeReachProbsValIter(MDP mdp, BitSet no, BitSet yes, boolean min, double init[], BitSet known) throws PrismException
	{
		...
		if (exactSol) {
			...
			res.exactSoln = exact.computeExactSol(mdp, min, yes, no, unknown, adv,
